...
func init() {
	tr.Template_FuncAdd("csrf_form", csrf_form)
	tr.Template_FuncAdd("csrf_form_param", csrf_form_param)
}
...
func Csrf_token(method string, hostname string, path string, url string, username string) (string, string) {
	...
	token := tr.Token_New(CSRF_TOKENNAME, username, tr.TOKEN_EXPIRATIONMINUTES, claims)
	tok, err := token.Sign()
	if err != nil {
		tr.Errf("Token Signing failed: %s", err.Error())
		return "", ""
	}
...
func csrf_input(cui PfUI, url string, method string) (o string) {
	...
	if tr.Debug {
		o += "<input type=\"hidden\" name=\"" + CSRF_TOKENNAME + "debug\" value=\"" + tr.HE(json) + "\" />\n"
	}
	return
}
...
func csrf_Check(cui PfUI, tok string) (ok bool) {
	...
	claims := &CSRFClaims{}
	_, err := tr.Token_Parse(tok, CSRF_TOKENNAME, claims)
	if err != nil {
