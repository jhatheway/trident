...
func h_webfinger(cui PfUI) {
	...
	if strings.Contains(res, "@") {
		...
		c := strings.Split(res, "@")
		if c[1] != tr.System_Get().PeopleDomain {
		...
		loc := u.Scheme + "://" + u.Host
		if loc != tr.System_Get().PublicURL {
			/* Not our public URL */
	...
	j.Subject = "acct:" + username + "@" + tr.System_Get().PeopleDomain
	j.Links = make([]jrdlnk, 0)

	if rel == spec {
		jl.Rel = spec
		jl.Href = tr.System_Get().PublicURL + "/oauth2/"
