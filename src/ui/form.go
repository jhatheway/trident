...
func init() {
	tr.Template_FuncAdd("pfform", pfform)
}
...
func TFErr(cui PfUI, str string) (o string, buttons string, neditable int, subs []string, multipart bool) {
	...
	return tr.HE("Problem encountered while rendering template"), "", 0, []string{}, false
}
...
func pfform_keyval(kvs keyval.KeyVals, val string) string {
	...
	for _, kv := range kvs {
		/* Gotcha */
		k := tr.ToString(kv.Key)
		if k == val {
			return tr.ToString(kv.Value)
		}
	}
...
func pfform_select(kvs keyval.KeyVals, def, idpfx, fname, opts string) (t string) {
	...
	for _, kv := range kvs {
		key := tr.ToString(kv.Key)
		val := tr.ToString(kv.Value)
 ...
 */
func pfformA(cui PfUI, section *string, idpfx string, objtrail []interface{}, obj interface{}, m interface{}, ptype tr.PType) (o string, buttons string, neditable int, subs []string, multipart bool) {
	var err error

	/* Prepend this object to the trail */
	objtrail = append([]interface{}{obj}, objtrail...)

	s, va := tr.StructReflect(obj)
	...
	oname := tr.StructNameObjTrail(objtrail)
	...
	for i := 0; i < s.NumField(); i++ {
		...
		ttype, dorecurse, compound := tr.PfType(f, v, false)
		if ttype == "ignore" {
			continue
		}

		if dorecurse {
			tt, bb, ne, s, mp := pfformA(cui, section, idpfx, objtrail, tr.StructRecurse(v), m, ptype)
			o += tt
			buttons += bb
			neditable += ne
			subs = append(subs, s...)
		...
		if vis_func != "" {
			ok, obj := tr.ObjHasFunc(objtrail, vis_func)
			if ok {
				res, err2 := tr.ObjFunc(obj, vis_func, f.Name)
				if err2 != nil {
		...
		ok, allowedit, err = tr.StructPermCheck(cui, lptype, objtrail, tr.PTypeWrap(f))
		if err != nil {
		...
		/* Try to translate it */
		label = tr.TranslateObj(cui, objtrail, label)
		...
		if keyval_func != "" {
			...
			ok, obj := tr.ObjHasFunc(objtrail, oc_func)
			if ok {
				context, err = tr.ObjFuncIface(obj, oc_func)
				if err != nil {
			...
			ok, obj = tr.ObjHasFunc(objtrail, keyval_func)
			if !ok {
				err = errors.New("Keyval function " + tr.StructNameObj(obj) + " " + keyval_func + "() not found")
				return
			}

			kvs_i, err = tr.ObjFuncIface(obj, keyval_func, context)
			if err == nil {
		...
		hint := f.Tag.Get("hint")
		hint = tr.TranslateObj(cui, objtrail, hint)
		placeholder := f.Tag.Get("placeholder")
		placeholder = tr.TranslateObj(cui, objtrail, placeholder)
		...
		omitempty := tr.IsTrue(f.Tag.Get("pfomitempty"))
		checkboxmode := tr.IsTrue(f.Tag.Get("pfcheckboxmode"))
		...
		if placeholder != "" {
			...
			opts += " placeholder=\"" + tr.HE(placeholder) + "\""
		}
		...
		case reflect.String:
			...
			case "bool":
				val := tr.IsTrue(v.Interface().(string))
				t += "<input "
				t += "name=\"" + fname + "\" "
				t += "id=\"" + idpfx + fname + "\" "
				t += "type=\"checkbox\" "
		...
		case reflect.Struct:
			...
			case "database/sql.NullBool":
				val := tr.IsTrue(tr.ToString(v.Interface()))
				t += pfform_bool(val, fname, idpfx, opts, allowedit)
				break
		...
		case reflect.Map:
			if checkboxmode {
				...
				for _, k := range v.MapKeys() {
					key := tr.ToString(k.Interface())
					ov := v.MapIndex(k)
					val := tr.ToString(ov.Interface())
		...
		case reflect.Slice:
			...
			for k := 0; k < v.Len(); k += 1 {
				...
				case "[]int":
					ttype = "number"
					val := tr.ToString(v.Index(k).Interface())
		...
		default:
			return TFErr(cui, "Field '"+fname+"' is an unknown type "+ftype.String()+": "+tr.StructNameT(f.Type))
		}
...
func pfform(cui PfUI, obj interface{}, m interface{}, editable bool) template.HTML {
	...
	if obj == nil {
		return tr.HEB("No object provided")
	}
	...
	_, _, fvalue, err := tr.StructDetails(cui, m, "message", tr.SD_None)
	if err == nil && fvalue != "" {
		/* Found it */
		id := tr.HE(idpfx + "message")
		o += "<li class=\"okay\">\n"
	...
	_, _, fvalue, err = tr.StructDetails(cui, m, "error", tr.SD_None)
	if err == nil && fvalue != "" {
		/* Found it */
		id := tr.HE(idpfx + "error")
		o += "<li class=\"error\">\n"
		o += "<label for=\"" + id + "\">&nbsp;</label>\n"
		o += "<span id=\"" + id + "\">\n"
		o += tr.HE(fvalue)
	...
	if err != nil {
		return tr.HEB("pfform head failed")
	}
