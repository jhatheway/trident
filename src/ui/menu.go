...
func NewPfUIMentry(URI string, Desc string, Perms tr.Perm, Fun PfUIFunc) PfUIMentry {
...
func (menu *PfUIMenu) AddPerms(uri string, perms tr.Perm) {
	for i, m := range menu.M {
		if m.URI == uri {
...
func (menu *PfUIMenu) DelPerms(uri string, perms tr.Perm) {
	for i, m := range menu.M {
		if m.URI == uri {
			menu.M[i].Perms &^= perms
			return
...
func (menu *PfUIMenu) SetPerms(uri string, perms tr.Perm) {
	for i, m := range menu.M {
		if m.URI == uri {
			menu.M[i].Perms = perms
			return
