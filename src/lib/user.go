...
func user_menu(ctx PfCtx, args []string) (err error) {
	menu := NewPfMenu([]PfMEntry{
		...
		{"password", user_pw_menu, 0, -1, nil, PERM_NONE, "Password commands"},
		...
		{"nominate", user_nominate, 5, 5, []string{"username", "email", "bio_info", "affiliation", "descr"}, PERM_USER, "Nominate New User"},
	})
...
type TriUser interface {
	PfUser
	IsNominator(ctx PfCtx, nom_name string) (ok bool)
	BestNominator(ctx PfCtx) (nom_name string, err error)
}

type TriUserS struct {
	PfUser `pfset:"self" pfget:"self"`
}

func NewTriUser() PfUser {
	return &TriUserS{PfUser: NewPfUser(nil, nil)}
}

func (user *TriUserS) IsNominator(ctx PfCtx, nom_name string) (ok bool) {
	cnt := 0
	...
	q := "SELECT COUNT(*) " +
		...
		"WHERE vouchor = $2"
	_ = DB.QueryRow(q, user.GetUserName(), nom_name).Scan(&cnt)

	return cnt == 0
}

func (user *TriUserS) BestNominator(ctx PfCtx) (nom_name string, err error) {
	q := "SELECT vouchor " +
		"FROM member_vouch mv " +
		"JOIN member_email me ON mv.vouchor = me.member " +
		"WHERE vouchee = $1 " +
		...
		"ORDER BY mv.entered " +
		"LIMIT 1"
	err = DB.QueryRow(q, user.GetUserName()).Scan(&nom_name)
	return
}

func user_pw_send(ctx PfCtx, is_reset bool, nom_name string) (err error) {
	var user_email PfUserEmail
	var nom_email PfUserEmail
	var pw PfPass
	...
	/* Make sure the name is mostly sane */
	nom_name, err = Chk_ident("UserName", nom_name)
	if err != nil {
		return
	}
	...
	err = ctx.SelectUser(nom_name, PERM_USER_NOMINATE)
	if err != nil {
		return
	}
	...
	err = ctx.SelectUser(username, PERM_USER)
	if err != nil {
		return
	}
...
func user_pw_reset(ctx PfCtx, args []string) (err error) {
...
func user_nominate(ctx PfCtx, args []string) (err error) {
	username := args[0]
	email := args[1]
	bio_info := args[2]
	affiliation := args[3]
...
/* JAMES TODO Not sure how to handle this yet
func user_pw_menu(ctx PfCtx, menu *PfMenu) {
*/
func user_pw_menu(ctx PfCtx, args []string) (err error) {
/*
	m := []PfMEntry{
		{"reset", user_pw_reset, 1, 2, []string{"username", "nominator"}, PERM_USER, "Send a recovery password split between the user and a nominator"},
	}
