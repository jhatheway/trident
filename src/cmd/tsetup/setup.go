...
func Setup(tname string, ldname string, appname string, version string, copyright string, website string, app_schema_version int, env_server string, server string, newctx tr.PfNewCtx) {
	var err error = nil
	var confroot string
	var verbosedb bool
	var force bool
	...
	/* Load configuration */
	err = tr.Config.Load(ldname, confroot)
	if err != nil {
	...
	/* Init DB */
	tr.DB_Init(verbosedb)
	tr.DB_SetAppVersion(app_schema_version)
	...
	case "upgrade_db":
		err = tr.System_db_upgrade()
		if err != nil {
			fmt.Println("Error: " + err.Error())
			return
		}
		err = tr.App_db_upgrade()
		if err != nil {
			fmt.Println("Error: " + err.Error())
			return
		}
	...
	case "adduser":
		if len(args) != 3 {
			...
			err = tr.System_adduser(args[1], args[2])
		}
		break
	...
	case "setpassword":
		if len(args) != 3 {
			...
			err = tr.System_setpassword(args[1], args[2])
		}
		break
